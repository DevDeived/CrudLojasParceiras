...
function salvar() {
  ...
  lojasParceiras.push(lojaParceira); 
...
function excluir(idProcurado) {
  for (let contador = 0; contador < lojasParceiras.length; contador++) {
    let lojaParceira = lojasParceiras[contador];

    if (lojaParceira.id == idProcurado) {
      if (confirm(`Você tem certeza que deseja excluir ${lojaParceira.lojas}"?`)) {
        lojasParceiras.splice(contador, 1);
        listar();
...
function salvarEdicao() {
  ...
  lojasParceiras[index].lojas = document
    .getElementById("editar-loja")
    .value.trim();
  lojasParceiras[index].email = document
    .getElementById("editar-email")
    .value.trim();
  lojasParceiras[index].telefone = document
